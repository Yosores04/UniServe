...
export function EntrepreneurPortal({
  ...
  const [orderFilter, setOrderFilter] = useState<
    "All" | "New" | "Active" | "Completed"
  >("All");
  const [newItem, setNewItem] = useState("");
  const selectedStore = state.stores[0];
  const storeOrders = state.orders.filter(
    (order) =>
      order.storeId === selectedStore.id &&
      (orderFilter === "All" ||
        (orderFilter === "New"
          ? order.storeStatus === "New"
          : orderFilter === "Completed"
            ? order.storeStatus === "Completed"
            : order.storeStatus !== "Completed")),
  );
        ...
        <div className="dashboard-grid">
          <Panel className="span-5">
            ...
            <div className="store-summary">
              ...
              <span className="store-hours">
                Open {selectedStore.operatingHours}
              </span>
            </div>
            ...
            <div className="store-controls">
              ...
              >
                {selectedStore.status === "Open" ? "Close store" : "Open store"}
              </button>
          ...
          <Panel className="span-7">
            ...
            <div className="filter-row">
              {(["All", "New", "Active", "Completed"] as const).map(
                (filter) => (
                  <button
                    className={
                      orderFilter === filter
            ...
            <div className="order-list">
                  ...
                  <div className="status-actions">
                    {order.storeStatus === "New" && (
                      <button
                        ...
                        onClick={() =>
                          setState((current) =>
                            acceptStoreOrder(current, order.id),
                          )
                        }
                      >
                        Accept order
                    ...
                    {(
                      ...
                      <button
                        ...
                        onClick={() =>
                          setState((current) =>
                            setStoreOrderStatus(current, order.id, status),
                          )
                        }
                      >
                        {status}
              ...
              {storeOrders.length === 0 && (
                <EmptyState text="No orders waiting for this store." />
              )}
            </div>
          </Panel>
          ...
          <Panel className="span-12">
            <PanelHeader
              ...
              title="Campus entrepreneur snapshot"
            />
